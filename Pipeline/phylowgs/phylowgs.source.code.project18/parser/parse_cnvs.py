...
class Team18Parser(CnvParser):
  def __init__(self, filename, cellularity):
  ...
  def parse(self):
    ...
    with open(self._filename) as file:
      ...
      for record in reader:
        ...
        cnv['end'] = int(record['end'])
        cnv['minor_cn'] = int(record['minor_cn'])
        cnv['major_cn'] = int(record['major_cn'])
        cnv['cellular_prevalence'] = self._cellularity

        cn_regions[chromosome].append(cnv)
...
def main():
  ...
  parser.add_argument('-f', '--cnv-format', dest='input_type', required=True, choices=('team18', 'battenberg', 'battenberg-smchet', 'titan'),
  ...
  elif args.input_type == 'team18':
    parser = Team18Parser(args.cnv_file, cellularity) 
  else:
