import os
import numpy as np
def intergret(firstfilename,secondfilename,purity):
    ...
    for item in seg_content[1:]:
        add_item = np.array([item[3],item[4],item[5]])
        minrange = int(item[1])
        maxrange = int(item[2])
        chrindex = item[0]
    ...
    purityRow = (np.ones(final_result.shape[0]) * purity)[:,None]
    final_result = np.hstack((final_result,purityRow))
    label = ['mutation_id','ref_counts','var_counts','normal_cn','minor_cn','major_cn','purity']
